...
export default function KycPage() {
    ...
    const upload = async () => {
        if (!nationalId || !selfie || !nationalCode || !birthDate || !address || !postalCode) {
            toast(lang === 'fa' ? 'تمام مدارک و اطلاعات فرم الزامی است' : 'All fields and files are required', 'error');
            return;
        }
        setLoading(true);
        try {
            await kycApi.uploadDocuments(uid, { nationalId, selfie, nationalCode, birthDate, address, postalCode }, 'STANDARD');
            toast(lang === 'fa' ? 'مدارک و اطلاعات بارگذاری شد' : 'Documents uploaded', 'success');
            kyc.refetch();
        } catch (err) {
                ...
                <Card className="lg:col-span-2">
                    ...
                    {kyc.loading ? <Spinner /> : (
                        <>
                            ...
                            {/* Form fields for KYC details */}
                            <div className="mt-6 grid gap-4 sm:grid-cols-2">
                                <div>
                                    <label className="label">{lang === 'fa' ? 'کد ملی' : 'National Code'}</label>
                                    <input className="input" value={nationalCode} onChange={(e) => setNationalCode(e.target.value)} placeholder="مثلاً 1234567890" maxLength={10} />
                                </div>
                                <div>
                                    <label className="label">{lang === 'fa' ? 'تاریخ تولد' : 'Date of Birth'}</label>
                                    <input className="input" type="date" value={birthDate} onChange={(e) => setBirthDate(e.target.value)} />
                                </div>
                                <div>
                                    <label className="label">{lang === 'fa' ? 'کد پستی' : 'Postal Code'}</label>
                                    <input className="input" value={postalCode} onChange={(e) => setPostalCode(e.target.value)} placeholder="10 رقمی" maxLength={10} />
                                </div>
                                <div className="sm:col-span-2">
                                    <label className="label">{lang === 'fa' ? 'آدرس محل سکونت' : 'Home Address'}</label>
                                    <textarea className="input min-h-[80px]" value={address} onChange={(e) => setAddress(e.target.value)} placeholder={lang === 'fa' ? 'استان، شهر، خیابان، پلاک...' : 'Full address'} />
                                </div>
                            </div>
